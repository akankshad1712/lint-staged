...
const hasIntentToAddFiles = async (execGit) => {
  const diff = await execGit(['diff', '--raw'])

  return diff
    .split('\n')
    .some((line) => line.includes(':000000 000000'))
}
export class GitWorkflow {
  ...
  async prepare(ctx) {
    ...
    try {
      ...
      if (ctx.shouldBackup) {
        if (ctx.shouldHideUnstaged || ctx.shouldHideAll) {
  if (await hasIntentToAddFiles(this.execGit)) {
